...
A logarithm is the inverse operation of exponentiation, determining the exponent to which a fixed base must be raised to produce a given value. Use logarithms when solving equations where the unknown variable is located inside an exponent, or when converting exponential growth models into linear formats. The base of a logarithm must always be a positive number not equal to one, and you can only take the logarithm of strictly positive numbers.
...
$y = a^x \iff x = \log_a y$; the base must be positive and not equal to 1, and the argument must be strictly positive.

## Worked example — Rewriting to logarithmic form

Express $5^3 = 125$ in logarithmic form.

Here the base is 5, the exponent is 3, and the result is 125. So $3 = \log_5(125)$.

## Worked example — Evaluating a logarithm

Evaluate $\log_2 16$.

Rewrite: $\log_2 16 = x \iff 2^x = 16$. Since $2^4 = 16$, the answer is $4$.
