...
Energy is the fundamental "currency" of the universe, acting as the capacity to do work. From launching a massive roller coaster to its highest peak to the simple act of striking a match, energy is constantly being stored, transferred, and converted. Understanding how energy behaves—specifically that it can never be truly lost, only spread out—is essential for mastering physics and designing everything from efficient cars to sustainable power grids.

## Key definitions

- **Energy**: The capacity to do work (SI unit: joule, J).
- **Kinetic energy ($E_k$)**: The energy of a body due to its motion (SI unit: joule, J).
- **Potential energy**: Stored energy in a system due to its state, shape, or position.
- **Gravitational potential energy ($E_p$)**: Energy stored in a body due to its height from the ground (SI unit: joule, J).
- **Chemical potential energy**: Energy stored in a substance due to the position of the atoms or electrons.
- **Elastic (strain) energy**: Energy stored in a body due to its elastic deformation, such as a stretched spring.
- **Internal (thermal) energy**: Energy stored in a body due to its temperature.
- **Nuclear energy**: Energy released during a nuclear reaction, found in the nuclei of atoms.
- **Principle of Conservation of Energy**: Energy cannot be created or destroyed, but can be converted from one form to another or transferred from one body to another; the total amount remains constant.
- **Dissipation**: The process where energy becomes more spread out among objects and the surroundings, often as wasted thermal or sound energy.
...
Identify the energy conversion when a light bulb is connected to a dry cell.
...
If an object moves along a slope, use the vertical height change, not the length of the slope.
