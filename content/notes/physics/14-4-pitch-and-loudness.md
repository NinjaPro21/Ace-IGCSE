...
Every sound has a unique character: pitch relates to frequency, loudness relates to amplitude. A cathode-ray oscilloscope (c.r.o.) displays these as displacement–time graphs.

## Key definitions

- **Pitch**: How high or low a sound seems — related to **frequency** (unit: Hz).
- **Loudness**: How loud a sound seems — related to **amplitude** (unit: m).
- **Amplitude ($A$)**: Maximum displacement of a point on the wave from its rest position.
- **Frequency ($f$)**: Number of complete waves per second.
- **Period ($T$)**: Time for one complete wave (unit: s).
...
where $v$ is wave speed and $\lambda$ is wavelength.

## Graphs & diagrams

**Visualising on a c.r.o.**

- A microphone converts sound to an electrical signal; the c.r.o. shows displacement vs time.
- **Pitch**: more cycles in the same time → higher frequency → higher pitch.
...
**Analysing waveforms on a c.r.o.**

1. Check the time base is the same for both traces.
...
- Increasing amplitude (playing louder) does **not** change pitch (frequency).
- Since $v = f\lambda$, higher frequency means **shorter wavelength** if speed is constant.
- Loudness is subjective; amplitude is the physical measurement.

## Examiner tip

- Link **pitch ↔ frequency** and **loudness ↔ amplitude** explicitly.
- Convert period from milliseconds to seconds before using $f = 1/T$.
- Human hearing: **20 Hz to 20 000 Hz**.

## Quick check

If the number of complete waves per second increases, what happens to the pitch?
