...
Whether you are dropping a pen or watching a skydiver leap from a plane, gravity is the invisible force at work. In IGCSE Physics, understanding the acceleration of free fall allows you to predict how fast objects will drop and how long they take to reach the ground. Mastering this topic is essential for distinguishing between the idealized world of a vacuum and the real-world effects of air resistance that allow a parachutist to land safely.
...
Applies to objects in free fall near the Earth's surface where air resistance is negligible.

**Final speed in free fall (starting from rest)**

$v = g \times t$

Applies when an object is dropped from rest ($u = 0$).

**Distance travelled (Height)**

$h = \text{Area under the speed-time graph} = \frac{1}{2} \times v \times t$

Applies for uniform acceleration from rest.
...
Calculate final speed: Multiply gravity by the time taken ($v = g \times t$).

Determine height: Calculate the area of the triangle under your speed-time graph ($\text{Area} = \frac{1}{2} \times \text{base} \times \text{height}$).
...
Vector direction: Always state that acceleration due to gravity acts downward or towards the centre of the Earth.

## Quick check

In a vacuum, which hits the ground first if dropped together: a hammer or a feather?

What is the approximate value of the acceleration of free fall near Earth?

When air resistance equals the weight of a falling object, what is the resultant force?
