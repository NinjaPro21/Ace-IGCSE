...
$$
V_{\text{sphere}} = \frac{4}{3}\pi r^3 \quad \text{|} \quad \text{SA}_{\text{sphere}} = 4\pi r^2
$$
...
If given vertical height $h$, use $l = \sqrt{r^2 + h^2}$ to determine the slant height.
...
Calculate each component volume independently using their respective formulas (e.g., $V_{\text{cylinder}} = \pi r^2 h$ and $V_{\text{hemisphere}} = \frac{2}{3}\pi r^3$).
...
Be careful not to mix up the vertical height ($h$) and the slant height ($l$). If you use the wrong height value, the calculation breaks.
...
May 2021 Paper 41 Q8b: A solid metal cylinder has a radius of $3.5\text{ cm}$ and a height of $12\text{ cm}$. Calculate its total surface area.

Step 1: Identify parameters: $r = 3.5$, $h = 12$.
...
Step 1: Find the missing slant height $l$ when $2$.
