## Core idea

Differentiation finds $\frac{dy}{dx}$, the gradient at a point. Set $\frac{dy}{dx}=0$ to locate turning points.

## Key formulas

**Power rule**

$$
y = ax^n \Rightarrow \frac{dy}{dx} = nax^{n-1}
$$

**Linear function**

$$
y = mx + c \Rightarrow \frac{dy}{dx} = m
$$

**Turning point**

$$
\frac{dy}{dx} = 0
$$

## Graphs & diagrams

<div class="ace-physics-diagram"><svg viewBox="0 0 320 200" width="320" height="200" role="img" aria-label="Tangent to curve">
      <line x1="40" y1="160" x2="280" y2="160" stroke="#a8a29e"/>
      <line x1="74.28571428571428" y1="40" x2="74.28571428571428" y2="160" stroke="#a8a29e"/>
      <polyline points="40.0,156.7 50.3,158.4 60.6,159.5 70.9,160.0 81.1,159.9 91.4,159.2 101.7,157.9 112.0,156.0 122.3,153.5 132.6,150.4 142.9,146.7 153.1,142.4 163.4,137.5 173.7,132.0 184.0,125.9 194.3,119.2 204.6,111.9 214.9,104.0 225.1,95.5 235.4,86.4 245.7,76.7 256.0,66.4 266.3,55.5 276.6,44.0" fill="none" stroke="#5b8def" stroke-width="2.5"/>
      <line x1="88" y1="168" x2="225.14285714285714" y2="114.66666666666666" stroke="#b59a73" stroke-width="2"/>
      <circle cx="142.85714285714283" cy="146.66666666666666" r="4" fill="#b59a73"/>
      <text x="150.85714285714283" y="140.66666666666666" font-size="10" fill="#b59a73">tangent</text>
    </svg><p class="ace-physics-diagram__caption">Gradient of curve — draw tangent at a point; gradient = rise ÷ run (derivative at that $x$).</p></div>

## Steps / method

Write each term in index form (e.g. $\frac{2}{x} = 2x^{-1}$).

Differentiate term by term with the power rule.

Constants differentiate to $0$.

Substitute an $x$ when $2$ for turning points.

## Examiner tip

Convert fractions to negative indices before differentiating — do not differentiate $\frac{2}{x}$ as $\frac{2}{1}$.

## Quick check

At a turning point the tangent is horizontal, so $\frac{dy}{dx}=0$.

## Worked example — Example 1 (June 2023 P42 Q9a)

Find $\frac{dy}{dx}$ if $y = 2x^3 - 5x^2 + 4x - 7$.

$$
\frac{dy}{dx} = 6x^2 - 10x + 4
$$

## Worked example — Example 2 (Nov 2022 P41 Q10c)

Find the turning point of $y = x^2 - 6x + 5$.

$$
\frac{dy}{dx} = 2x - 6 = 0 \Rightarrow x = 3
$$

$$
y = 3^2 - 6(3) + 5 = -4 \Rightarrow (3,-4)
$$
