...
export function DuelsPanel() {
  ...
  return (
    <section className="ace-duels-hub ace-dashboard-card">
      <div className="ace-duels-hub__head">
        <div>
          <h2 className="ace-heading-serif">Quiz duels</h2>
          <p className="ace-body-text">Head-to-head on the same section — higher score wins.</p>
        </div>
        <span className="ace-duels-hub__badge">{live.length} live</span>
      </div>
        ...
        <div className="ace-insights-table">
          {live.map((d) => {
            ...
            return (
              <div
                key={d.id}
                className={['ace-insights-row', needsYou ? 'ace-insights-row--action' : ''].filter(Boolean).join(' ')}
              >
                <div className="ace-insights-row__main">
                  <span className={['ace-insights-row__subject', subjectClass].filter(Boolean).join(' ')}>
                    {subjectName}
                  </span>
                  <span className="ace-insights-row__title">
      ...
      {recent.length > 0 && (
        <div className="ace-duels-hub__recent">
          <h3 className="ace-duels-hub__recent-title">Recent results</h3>
          <ul className="ace-duels-hub__recent-list">
            {recent.map((d) => {
              const other = d.challengerUid === user.id ? d.opponentUid : d.challengerUid
              const { scope, diff } = describe(d)
              const won = d.winnerUid === user.id
              return (
                <li key={d.id} className="ace-duel-result">
                  <span>
                    {won ? 'Won' : 'Lost'} vs {names[other] ?? '…'} · {scope} · {diff}
                  </span>
                  <span className={won ? 'ace-duel-result--win' : 'ace-duel-result--loss'}>
                    {won ? 'Victory' : 'Defeat'}
                  </span>
                </li>
              )
