#!/usr/bin/env node
/**
 * Repair quiz JSON for Add Math Ch.2 topics only.
 *
 * WARNING: Runs prepareMathContent() which can over-wrap bare math as display
 * math ($$\n…\n$$). Prefer manual JSON fixes + audit. Use only on raw imports.
 */
import fs from 'fs'
...
const chapter = JSON.parse(
  fs.readFileSync(
    path.join(__dirname, '../content/chapters/add-maths-0606/ch02-simultaneous-equations-and-quadratics.json'),
    'utf8',
  ),
)
...
console.log(`Repaired ${files} Ch.2 quiz files (${questions} questions).`)
